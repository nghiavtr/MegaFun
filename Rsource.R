...
crpcount = function(filename, weight_thres=0.9)
{
  ...
  lines = gsub("\'", "", lines)
  rawcount  = read.table(textConnection(lines), header=TRUE, sep='\t')
  ...
  CRP.eqID = paste(CRP.names, unlist(CRP.eq), sep = "@") 
  ...
  # eq not exist in X matrix
  # get the biggest set overlap with X matrix
  # eq_not_exist = setdiff(eqID2, CRP.eqID)
  # new_id = sapply(eq_not_exist, function(s){
  #   temp = unlist(strsplit(s, "@"))
  #   pick = which(temp[1] == CRP.names)
  #   exist_crp_eq_all = unlist(CRP.eq)[pick]
  #   names(exist_crp_eq_all) = NULL
  #   exist_crp_eq = lapply(exist_crp_eq_all, function(s){
  #     unlist(strsplit(s, "\\*"))
  #   })
  #
  #
  #   current_crp_eq = unlist(strsplit(temp[2], "\\*"))
  #   n_overlap = sapply(exist_crp_eq, function(s){
  #     length(intersect(s, current_crp_eq))
  #   })
  #   id = exist_crp_eq_all[which.max(n_overlap)]
  #   paste0(temp[1], "@", id)
  # })
  # count_for_new_id = eqCount2[match(eq_not_exist, eqID2)]
  #
  # count_for_new_id = tapply(count_for_new_id, new_id, sum)
  #
  # ycount[names(count_for_new_id)] = ycount[names(count_for_new_id)] + count_for_new_id
  #
